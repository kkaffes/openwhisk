...
class LateBindingBalancer(
  ...
  val schedulingState = LateBindingBalancerState()(lbConfig)
...
object LateBindingBalancer extends LoadBalancerProvider {
...
case class LateBindingBalancerState(
  ...
  private var _blackboxStepSizes: Seq[Int] = LateBindingBalancer.pairwiseCoprimeNumbersUntil(0),
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    val logDetail = if (oldSize != newSize) {
      _managedStepSizes = LateBindingBalancer.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = LateBindingBalancer.pairwiseCoprimeNumbersUntil(blackboxes)
