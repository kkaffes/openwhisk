...
class LocalityBalancer(
  ...
  val schedulingState = LocalityBalancerState()(lbConfig)
  ...
  override protected[loadBalancer] def processCompletion(aid: ActivationId,
   ...
   activationsPerNamespace.get(entry.namespaceId).foreach(_.decrement())
        ...
        if (!forced) {
          ...
          scheduleQueuedActivation(invoker.toInt, entry.fullyQualifiedEntityName.toString())
          ...
          schedulingState.increaseCapacityOrDecreaseLoad(invoker.toInt, entry.fullyQualifiedEntityName.toString())
          logging.warn(
            this,
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
   ...
   // Try to find a warm action in a non-empty node with available capacity.
   invokerIndex = schedulingState.findWarmAction(action.fullyQualifiedName(true).toString())
  ...
  def scheduleQueuedActivation(invokerId: Int, actionName: String) = {
    val actionType = "managed"
    val activation = schedulingState.returnPendingActivation()
    activation match {
      case Right(x) => {
         schedulingState.increaseCapacityOrDecreaseLoad(invokerId, actionName)
...
object LocalityBalancer extends LoadBalancerProvider {
 ...
 */
case class LocalityBalancerState(
  ...
  private var _warmActions: IndexedSeq[scala.collection.mutable.Map[String, Int]] = IndexedSeq.empty[scala.collection.mutable.Map[String, Int]], 
  ...
  private var _managedStepSizes: Seq[Int] = LocalityBalancer.pairwiseCoprimeNumbersUntil(0),
  private var _blackboxStepSizes: Seq[Int] = LocalityBalancer.pairwiseCoprimeNumbersUntil(0),
  ...
  def increaseCapacityOrDecreaseLoad(invokerId: Int, actionName: String) = {
    this.synchronized {
      _warmActions(invokerId)(actionName) = _warmActions(invokerId).get(actionName).getOrElse(0) + 1
  ...
  def findWarmAction(actionName: String) : Int = {
    var invokerIdx = -1;
    this.synchronized {
      // Try to find a non-empty Invoker with capacity with a warm action.
      var minCapacity = 9;
      for ( x <- 0 to _invokerCapacity.length - 1) {
        // Check if is there a warm container in a non-empty Invoker.
        if (_invokerCapacity(x) > 0) {
          if (_warmActions(x).get(actionName).getOrElse(-1) > 0) {
            if (_invokerCapacity(x) < minCapacity) {
              invokerIdx = x;
              minCapacity = _invokerCapacity(x);
            }
          }
        }
      }
      if (invokerIdx != -1) {
        _warmActions(invokerIdx)(actionName) = _warmActions(invokerIdx).get(actionName).getOrElse(0) - 1
        _invokerCapacity = _invokerCapacity.updated(invokerIdx,  _invokerCapacity(invokerIdx) - 1)
        return invokerIdx
      }

      // If not, try to find any Invoker with available capacity.
      for ( x <- 0 to _invokerCapacity.length - 1) {
        if (invokerCapacity(x) > 0) {
          _invokerCapacity = _invokerCapacity.updated(x,  _invokerCapacity(x) - 1)
          return x
        }
      }

      var firstIndex = _invokerLoads.indexOf(_invokerLoads.min)
      if (_invokerLoads(firstIndex) == 92) {
        return -1
      }
      var leastLoadedIndex = firstIndex
      do {
        if (_warmActions(leastLoadedIndex).get(actionName).getOrElse(-1) > 0) {
          _warmActions(leastLoadedIndex)(actionName) = _warmActions(leastLoadedIndex).get(actionName).getOrElse(0) - 1
          _invokerLoads = _invokerLoads.updated(leastLoadedIndex,  _invokerLoads(leastLoadedIndex) + 1)
          return leastLoadedIndex
        }
        leastLoadedIndex = _invokerLoads.indexOf(_invokerLoads.min, leastLoadedIndex)
      } while (leastLoadedIndex != firstIndex)
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    if (oldSize != newSize) {
      ...
      _warmActions = IndexedSeq.fill(newSize)(scala.collection.mutable.Map[String, Int]())
    }
    ...
    val logDetail = if (oldSize != newSize) {
      _managedStepSizes = LocalityBalancer.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = LocalityBalancer.pairwiseCoprimeNumbersUntil(blackboxes)
